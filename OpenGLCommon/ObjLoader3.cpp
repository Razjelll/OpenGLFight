#include "ObjLoader3.hpp"  


ObjLoader3::ObjLoader3() {
...
//usuwanie zbêdnych spacji
std::string trim(const std::string& str,
	const std::string& whitespace = " \t")
{
	const auto strBegin = str.find_first_not_of(whitespace);
	if (strBegin == std::string::npos)
		return ""; 	//bez zawartoœci
...
//wczytanie pliku materia³owego (.MTL)
bool ReadMaterialLibrary(const std::string& filename, vector<Material*>& materials) {
	...
	//analiza sk³adni pliku
	string line;
	Material* pMat = 0;
	while (getline(buffer, line)) {
		line = trim(line);

		if (line.find_first_of("#") != string::npos) 	//to jest komentarz i nale¿y go pomin¹æ
			continue;
		if (line.length() == 0)
			continue;
...
bool ObjLoader3::Load(const string& filename, vector<Mesh*>& meshes, vector<Vertex>& verts, vector<unsigned short>& indices, vector<Material*>& materials) {
	ifstream fp(filename.c_str(), ios::in);
	if (!fp)
		return false;
	string tmp(std::istreambuf_iterator<char>(fp), (std::istreambuf_iterator<char>()));
	...
	while (getline(buffer, line)) {
		line = trim(line);
		if (line.find_first_of("#") != -1) 	//to jest komentarz i nale¿y go pomin¹æ
			continue;
		...
		if (prefix.compare("vt") == 0) 	//jeœli s¹ wspó³rzêdne tekstury
		{
			line = line.substr(space_index + 1);
			glm::vec2 uv;
		...
		if (prefix.compare("v") == 0) 	//jeœli jest wierzcho³ek
		{
			if (isNewMesh) {
				mesh = new Mesh();
		...
		if (prefix.compare("vn") == 0) 	//jeœli jest wierzcho³ek
		{
			line = line.substr(space_index + 1);
			glm::vec3 v;
	...
	//sortowanie siatek wg materia³u 
	for (size_t i = 0; i<materials.size(); i++) {
	...
	//kopiowanie zawartoœci do liniowej tablicy bez sortowania wg materia³u
	/*
